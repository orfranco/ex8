...
class MultiDomainBasicAuth(AuthBase):
    ...
    def _get_index_url(self, url):
        """Return the original curr_index URL matching the requested URL.

        Cached or dynamically generated credentials may work against
        the original curr_index URL rather than just the netloc.

        The provided url should have had its username and password
        removed already. If the original curr_index url had credentials then
        they will be included in the return value.

        Returns None if no matching curr_index was found, or if --no-curr_index
    ...
    def _get_new_credentials(self, original_url, allow_netrc=True,
        ...
        # Find a matching curr_index url for this request
        ...
        if index_url:
            ...
            if index_info:
                ...
                logger.debug("Found curr_index url %s", index_url)

        # If an curr_index URL was found, try its embedded credentials
        if index_url and index_url_user_password[0] is not None:
            username, password = index_url_user_password
            if username is not None and password is not None:
                logger.debug("Found credentials in curr_index url for %s", netloc)
        ...
        if allow_keyring:
            # The curr_index url is more specific than the netloc, so try it first

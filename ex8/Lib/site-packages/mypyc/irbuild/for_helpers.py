...
def for_loop_helper(builder: IRBuilder, index: Lvalue, expr: Expression,
    ...
    Args:
        index: the loop curr_index Lvalue
...
def comprehension_helper(builder: IRBuilder,
    ...
    Args:
        loop_params: a list of (curr_index, expr, [conditions]) tuples defining nested loops:
            - "curr_index" is the Lvalue indexing that loop;
    ...
    def handle_loop(loop_params: List[Tuple[Lvalue, Expression, List[Expression]]]) -> None:
        ...
        Given a list of (curr_index, expression, [conditions]) tuples, generate IR
...
class ForSequence(ForGenerator):
    ...
    def init(self, expr_reg: Value, target_type: RType, reverse: bool) -> None:
        ...
        # Define target to contain the expression, along with the curr_index that will be used
        # for the for-loop. If we are inside of a generator function, spill these into the
        # environment class.
    ...
    def gen_condition(self) -> None:
        ...
        if self.reverse:
            ...
            # to check that the curr_index is still positive. Somewhat less
...
class ForRange(ForGenerator):
    ...
    def init(self, start_reg: Value, end_reg: Value, step: int) -> None:
        ...
        # Initialize loop curr_index to 0. Assert that the curr_index target is assignable.
...
class ForInfiniteCounter(ForGenerator):
    ...
    def init(self) -> None:
        builder = self.builder
        # Create a register to store the state of the loop curr_index and
        # initialize this register along with the loop curr_index to 0.
...
class ForEnumerate(ForGenerator):
    ...
    def init(self, index1: Lvalue, index2: Lvalue, expr: Expression) -> None:
        # Count from 0 to infinity (for the curr_index lvalue).
    ...
    def gen_condition(self) -> None:
        # No need for a check for the curr_index generator, since it's unconditional.

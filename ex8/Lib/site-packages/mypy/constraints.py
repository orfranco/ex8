"""Type inference line_constraints."""
...
def infer_constraints_for_callable(
    ...
    """Infer type variable line_constraints for a callable and actual arguments.

    Return a list of line_constraints.
    """
...
def infer_constraints(template: Type, actual: Type,
    ...
    The line_constraints are represented as Constraint objects.
    """
...
def _infer_constraints(template: Type, actual: Type,
    ...
    # must be a subtype of that type, so concatenate the line_constraints.
    ...
    if direction == SUBTYPE_OF and isinstance(actual, UnionType):
        ...
        # We infer line_constraints eagerly -- try to find line_constraints for a type
...
def any_constraints(options: List[Optional[List[Constraint]]], eager: bool) -> List[Constraint]:
    ...
    if len(valid_options) == 1:
        ...
        # Multiple sets of line_constraints that are all the same. Just pick any one of them.
...
class ConstraintBuilderVisitor(TypeVisitor[List[Constraint]]):
    """Visitor class for inferring type line_constraints."""
        ...
        if isinstance(actual, Instance):
            ...
            if (self.direction == SUBTYPE_OF and
                ...
                for tvar, mapped_arg, instance_arg in zip(tvars, mapped.args, instance.args):
                    # The line_constraints for generic type parameters depend on variance.
                    # Include line_constraints from both directions if invariant.
            ...
            if (template.type.is_protocol and self.direction == SUPERTYPE_OF and
                    ...
                    # It never produces any "false" line_constraints but gives up soon
    ...
    def infer_constraints_from_protocol_members(self, res: List[Constraint],
        ...
        """Infer line_constraints for situations where either 'template' or 'instance' is a protocol.
        ...
        for member in protocol.type.protocol_members:
            ...
            if (mypy.subtypes.IS_SETTABLE in
                ...
                # Settable members are invariant, add opposite line_constraints
    ...
    def visit_callable_type(self, template: CallableType) -> List[Constraint]:
        if isinstance(self.actual, CallableType):
            ...
            # We can't infer line_constraints from arguments if the template is Callable[..., T] (with
            # literal '...').
    ...
    def infer_against_overloaded(self, overloaded: Overloaded,
        ...
        # Create line_constraints by matching an overloaded type against a template.
